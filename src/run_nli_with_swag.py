...
from swa_gaussian.swag import data, losses  # , utils
from swa_gaussian.swag.posteriors import SWAG
import utils  # overrides some methods from swa_gaussian.utils

#from nli_with_transformers import models
from nli_with_transformers.data import get_nli_dataset
...
#print("Preparing model %s" % args.model)
###tokenizer, model = models.get_model(args)
###model_copy = copy.deepcopy(model)
#model_cfg = LangModel()
#model = model_cfg.base(args)
...
print("Loading dataset %s" % (args.data_path if args.data_path else "snli"))
train_loader, dev_loader, test_loader = get_nli_dataset(args, tokenizer)
...
for epoch in range(start_epoch, args.epochs):
    ...
    if (args.swa and (epoch + 1) > args.swa_start) and args.cov_mat:
        train_res = utils.train_epoch(train_loader, model, optimizer, cuda=use_cuda)
    else:
        train_res = utils.train_epoch(train_loader, model, optimizer, cuda=use_cuda)
    ...
    if (
        ...
        # sgd_preds, sgd_targets = utils.predictions(test_loader, model)
        ...
        if (
            ...
            swag_res = utils.eval(test_loader, swag_model)
        else:
            swag_res = {"loss": None, "accuracy": None}
